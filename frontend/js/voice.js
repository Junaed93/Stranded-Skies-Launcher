...
function initVoice() {
  console.log("[Voice] Initializing with raw WebSocket...");
  createVoiceIndicator();

  const wsUrl = CONFIG.API_URL.replace(/^http/, "ws") + "/ws/voice";
...
function sendSignal(data) {
  if (!voiceSocket || voiceSocket.readyState !== WebSocket.OPEN) {
    console.warn("[Voice] WebSocket not connected, cannot send signal");
    return;
  }
  ...
  console.log(
    ...
    data.type,
    data.targetId ? "to:" + data.targetId : "(broadcast)",
  );
...
async function startVoice() {
  ...
  if (!isVoiceConnected) {
    console.warn("[Voice] Not connected to voice server");
    return;
  }
  isVoiceActive = true;
  ...
  try {
    ...
    console.log(
      "[Voice] Microphone access granted, tracks:",
  ...
  try {
    if (pc.signalingState === "have-local-offer") {
      ...
      if (pendingCandidates[data.senderId] || pendingCandidates["broadcast"]) {
        const candidates = (pendingCandidates[data.senderId] || []).concat(
          pendingCandidates["broadcast"] || [],
        );
        for (const candidate of candidates) {
          await pc.addIceCandidate(new RTCIceCandidate(candidate));
          console.log(
            "[Voice] Applied pending ICE candidate from answer handler",
          );
        }
        delete pendingCandidates[data.senderId];
...
if (unityIframe) {
  unityIframe.onload = function () {
    try {
      const iframeDoc =
        unityIframe.contentDocument || unityIframe.contentWindow.document;
...
// Voice is initialized after room selection in index.html
// initVoice() is called from there via connectVoice()
